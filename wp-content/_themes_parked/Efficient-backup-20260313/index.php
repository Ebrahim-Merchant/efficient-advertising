...
<div class="homebaner">
  ...
  <!-- Static background (same for all slides) -->
  <img class="carousel-static-bg no-lazyload" data-skip-lazy="1" src="<?php echo esc_url( content_url( 'uploads/2022/06/bak-1.png' ) ); ?>" alt="">

  <!-- Static man + text overlay (single instance, animated via JS on each transition) -->
  <div class="folowup" id="carousel-man">
    <img class="no-lazyload" data-skip-lazy="1" fetchpriority="high" src="<?php echo esc_url( content_url( 'uploads/2022/07/banda.png' ) ); ?>" alt="">
  </div>
  <div class="contentleft text-center" id="carousel-text">
    <span class="tisda" id="carousel-label"></span><br>
    <a href="<?php echo esc_url( home_url( '/contact-us/' ) ); ?>"><button class="btn contus"><i class="fa fa-arrow-circle-right" aria-hidden="true"></i> Contact Us</button></a>
  </div>

  <!-- Carousel: only product images cycle -->
  <div id="carousel" class="carousel slide carousel-fade" data-ride="carousel" data-interval="5000">
    <div class="carousel-inner">
      <?php
        /* Same 7 categories shown in the header navigation */
        $_cats = get_terms( array(
            'taxonomy'   => 'product_category',
            'hide_empty' => true,
            'include'    => array( 4, 5, 10, 7, 9, 6, 15 ),
            'orderby'    => 'include',
        ) );
        $count = 0;
        if ( ! empty( $_cats ) && ! is_wp_error( $_cats ) ) :
        foreach ( $_cats as $_cat ) :
            ...
            if ( ! $_q->have_posts() ) continue;
            $_q->the_post();
            $thumb = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'full' );
            if ( ! $thumb || ! $thumb[0] ) { wp_reset_postdata(); continue; }

            /* Override carousel image for specific categories */
            $_override_imgs = array(
                6  => content_url( 'uploads/2026/03/car-wrap-vehicle-branding.jpg' ),   // Vehicle Branding
                9  => content_url( 'uploads/2025/04/Trade-Show-Booth-Deemed-University-1000-x-1000-px.jpg' ), // Exhibition & Event
            );
            $_img_src = isset( $_override_imgs[ $_cat->term_id ] ) ? $_override_imgs[ $_cat->term_id ] : $thumb[0];
      ?>
      <div class="<?php echo ( $count === 0 ) ? 'active ' : ''; ?>item" data-cat-name="<?php echo esc_attr( $_cat->name ); ?>">
        <div class="ablop_sw">
          <div class="fixsloew">
            <a href="<?php the_permalink(); ?>"><img class="productimg_ee no-lazyload" data-skip-lazy="1" src="<?php echo esc_url( $_img_src ); ?>" alt="<?php echo esc_attr( $_cat->name ); ?> Printing Dubai - Efficient Advertising"></a>
          </div>
        </div>
      </div>
      <?php
  ...
  (function(){
    document.addEventListener('DOMContentLoaded', function(){
      var man   = document.getElementById('carousel-man');
      ...
      if (!man || !text || !inner) return;

      /* Build ordered array of category names from the slide data attributes */
      var slides = inner.querySelectorAll('.item');
      var names  = [];
      for (var s = 0; s < slides.length; s++) {
        names.push(slides[s].getAttribute('data-cat-name') || '');
      }

      /* Set initial label from first (active) slide */
      if (label && names.length) label.textContent = names[0];
      ...
      new MutationObserver(function(mutations){
        for (var i = 0; i < mutations.length; i++){
          ...
          var newVal   = mutations[i].target.className;
          var target   = mutations[i].target;
          var getsNext = newVal.indexOf('next') > -1 && oldVal.indexOf('next') < 0;
          var getsPrev = newVal.indexOf('prev') > -1 && oldVal.indexOf('prev') < 0;

          if (getsNext || getsPrev) {
            /* Find which slide index is becoming active */
            for (var s = 0; s < slides.length; s++) {
              if (slides[s] === target) { nextIndex = s; break; }
            }
            clearTimeout(timer);
            var textOut = isMobile() ? 'translateX(-50%) translateY(12px)' : 'translateY(12px)';
            var textIn  = isMobile() ? 'translateX(-50%) translateY(0)'    : 'translateY(0)';
            /* Fade out immediately */
            man.style.transition  = 'opacity 0.25s ease, transform 0.25s ease';
            man.style.opacity     = '0';
            man.style.transform   = 'translateY(12px)';
            text.style.transition = 'opacity 0.25s ease, transform 0.25s ease';
            text.style.opacity    = '0';
            text.style.transform  = textOut;
            /* Swap label text while invisible, then fade back in */
            timer = setTimeout(function(){
              if (label) label.textContent = names[nextIndex];
              man.style.transition  = 'opacity 0.35s ease, transform 0.35s ease';
              man.style.opacity     = '1';
              man.style.transform   = 'translateY(0)';
              text.style.transition = 'opacity 0.35s ease, transform 0.35s ease';
              text.style.opacity    = '1';
              text.style.transform  = textIn;
            }, 350);
            break;
          }
        }
