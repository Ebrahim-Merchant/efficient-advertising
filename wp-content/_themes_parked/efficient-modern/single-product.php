...
<section class="sp-pricing-section">
  <div class="container">
    <div class="sp-sec-hdr"><h2>Pricing Guide — <?php echo esc_html( $sp_title ); ?> Dubai</h2><p>Indicative prices. Final price depends on quantity, material &amp; finish. <strong>Contact us for your exact quote.</strong></p></div>
    <div class="sp-pricing-wrap">
      <table class="sp-pricing-tbl">
        <thead><tr><th>Size / Product</th><th>Material / Spec</th><th>Indicative Price</th><th>Order</th></tr></thead>
        <tbody>
          <?php foreach ( $pricing as $row ) : ?>
          <tr>
            <td><?php echo esc_html( $row['size'] ); ?></td>
            <td><?php echo esc_html( $row['mat'] ); ?></td>
            <td class="sp-price-cell"><?php echo esc_html( $row['price'] ); ?></td>
...
<style>
:root{--sp-primary:#8B1538;--sp-dark:#660f29;--sp-orange:#FF6B35;--sp-light:#f9fafb;--sp-border:#e5e7eb;--sp-text:#1f2937;--sp-muted:#6b7280;--sp-radius:10px;--sp-shadow:0 2px 12px rgba(0,0,0,.08)}
...
.sp-price-cell{font-weight:700;color:var(--sp-primary);white-space:nowrap}
