...
// Finch Gift Shop – Applied 2026-03-05
// ============================================================

/**
 * 1. Remove WordPress version number from all public outputs
 *    (generator meta tag, RSS feeds, script/style query strings)
 */
remove_action( 'wp_head', 'wp_generator' );

function efficient_remove_version_from_assets( $src ) {
    if ( strpos( $src, 'ver=' ) ) {
        $src = remove_query_arg( 'ver', $src );
    }
    return $src;
}
add_filter( 'style_loader_src',  'efficient_remove_version_from_assets', 9999 );
add_filter( 'script_loader_src', 'efficient_remove_version_from_assets', 9999 );

function efficient_remove_rss_version() { return ''; }
add_filter( 'the_generator', 'efficient_remove_rss_version' );

/**
 * 2. Disable WordPress emoji scripts and styles
 *    (saves an external HTTP request on every page)
 */
function efficient_disable_emojis() {
...
add_action( 'init', 'efficient_disable_emojis' );

/**
 * 3. Add HTTP security headers via PHP
 *    (works as fallback if .htaccess mod_headers is not available)
 */
function efficient_security_headers() {
    if ( ! is_admin() ) {
...
add_action( 'send_headers', 'efficient_security_headers' );

/**
 * 4. Force HTTPS for site URL and home URL
 */
function efficient_force_ssl( $url ) {
    if ( substr( $url, 0, 5 ) === 'http:' ) {
        $url = 'https:' . substr( $url, 5 );
    }
    return $url;
}
add_filter( 'site_url',       'efficient_force_ssl' );
add_filter( 'home_url',       'efficient_force_ssl' );
add_filter( 'content_url',    'efficient_force_ssl' );
add_filter( 'plugins_url',    'efficient_force_ssl' );
add_filter( 'theme_root_uri', 'efficient_force_ssl' );
add_filter( 'the_content',    'efficient_force_ssl' );

/**
 * 5. Remove WooCommerce scripts from non-WooCommerce pages
 *    (reduces JS/CSS payload on blog posts, about pages, etc.)
 */
function efficient_dequeue_wc_scripts_conditionally() {
...
add_action( 'wp_enqueue_scripts', 'efficient_dequeue_wc_scripts_conditionally', 99 );

/**
 * 6. Add Google Fonts preconnect for faster font loading
 */
function efficient_preconnect_fonts() {
    echo '<link rel="preconnect" href="https://fonts.googleapis.com">' . "\n";
    echo '<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>' . "\n";
    echo '<link rel="preconnect" href="https://cdnjs.cloudflare.com">' . "\n";
}
add_action( 'wp_head', 'efficient_preconnect_fonts', 1 );

/**
 * 7. Fix WooCommerce "Related Products" (You May Also Like) visibility
 *    Ensure related products always show, with a fallback to recent products
 */
function efficient_fix_related_products( $args ) {
    // Increase related product limit and use both categories and tags for matching
    $args['posts_per_page'] = 4;
    $args['orderby']        = 'rand';
    return $args;
}
add_filter( 'woocommerce_output_related_products_args', 'efficient_fix_related_products' );

// Enable related products even when no tags/categories match
...
/**
 * 8. Disable XML-RPC (security risk, not needed for this site)
 */
add_filter( 'xmlrpc_enabled', '__return_false' );

// Also remove X-Pingback header
function efficient_remove_x_pingback( $headers ) {
    unset( $headers['X-Pingback'] );
    return $headers;
}
add_filter( 'wp_headers', 'efficient_remove_x_pingback' );

/**
 * 9. Remove unnecessary links from wp_head
 */
...
/**
 * 10. LocalBusiness Schema – structured data for Finch Gift Shop
 *     Renders JSON-LD on the homepage for Google rich results
 */
function efficient_localbusiness_schema() {
    ...
    <script type="application/ld+json">
    {
      ...
      "@type": "Florist",
      "name": "Finch Gift Shop",
      "url": "https://finchgiftshop.com",
      "logo": "https://finchgiftshop.com/wp-content/uploads/2025/02/finch-logo.png",
      "image": "https://finchgiftshop.com/wp-content/uploads/2025/02/finch-logo.png",
      "description": "Finch Gift Shop in North York, Toronto offers exquisite floral arrangements, personalized gifts, and luxury gift hampers for all occasions. Order online or via delivery partners.",
      "telephone": "+14167301028",
      "email": "info@finchgiftshop.com",
      "address": {
        "@type": "PostalAddress",
        "streetAddress": "5714 Yonge Street",
      ...
      "hasMap": "https://maps.app.goo.gl/5nJQrstUtACSDS3Z9",
      "priceRange": "$$",
      "servesCuisine": [],
      "sameAs": [
        "https://www.facebook.com/FinchGiftShop",
        "https://www.instagram.com/finchgiftshop/",
        "https://www.skipthedishes.com/finch-gift-shop-yonge",
        "https://www.doordash.com/en-CA/store/finch-gift-shop-toronto-24712946/",
        "https://www.ubereats.com/ca/store/finch-gift-shop/1yLeXxWwXRS34DYFpRua6A"
      ]
    }
    </script>
    <?php
}
add_action( 'wp_head', 'efficient_localbusiness_schema' );

/**
 * 11. WooCommerce – hide "Out of Stock" from catalog displays
 *     (also enforced via wp option update, this is a code-level fallback)
 */
add_filter( 'woocommerce_product_is_visible', function( $visible, $product_id ) {
    $product = wc_get_product( $product_id );
    if ( $product && ! $product->is_in_stock() ) {
        $hide_oos = get_option( 'woocommerce_hide_out_of_stock_items' );
        if ( 'yes' === $hide_oos ) {
            return false;
        }
    }
...
/**
 * 12. Fix nav menu items that use href="#" fallback
 *     Maps top-level category labels to their real WooCommerce URLs.
 *     This applies at render time so links work for both users and crawlers.
 */
function efficient_fix_nav_href_hash( $item_output, $item, $depth, $args ) {
    if ( empty( $item->url ) || '#' === $item->url || '' === $item->url ) {
        $label_to_url = array(
            'Birthday'    => '/category/birthday/all-birthday-gifts/',
            'Anniversary' => '/category/anniversary/all-anniversary/',
            'Flowers'     => '/category/flowers/all-flower-collection/',
            'Combo'       => '/category/flowers/combo/',
            'Balloons'    => '/category/balloons/all-balloons/',
            'Chocolates'  => '/category/chocolates/all-chocolates/',
            'Plants'      => '/category/plants/all-plants/',
            'More Gifts'  => '/shop/',
        );
        $title = $item->title;
        if ( isset( $label_to_url[ $title ] ) ) {
            $new_url = home_url( $label_to_url[ $title ] );
            // Swap href="#" in the rendered output
            $item_output = str_replace( 'href="#"', 'href="' . esc_url( $new_url ) . '"', $item_output );
            $item_output = str_replace( "href='#'", "href='" . esc_url( $new_url ) . "'", $item_output );
        }
    }
    return $item_output;
}
add_filter( 'walker_nav_menu_start_el', 'efficient_fix_nav_href_hash', 10, 4 );
