...
/**
 * Modern Asset Pipeline: Enqueue external assets instead of manual <script> tags.
 */
add_action( 'wp_enqueue_scripts', function () {
    // 1. CSS
    wp_enqueue_style( 'bootstrap', get_template_directory_uri() . '/css/bootstrap.min.css', [], '3.3.7' );
    wp_enqueue_style( 'animate', get_template_directory_uri() . '/css/animate.css', [], '3.5.2' );
    wp_enqueue_style( 'owl-carousel', get_template_directory_uri() . '/css/owl.carousel.min.css', [], '2.3.4' );
    wp_enqueue_style( 'owl-theme', get_template_directory_uri() . '/css/owl.theme.default.min.css', ['owl-carousel'], '2.3.4' );
    wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css/font-awesome.min.css', [], '4.7.0' );
    wp_enqueue_style( 'legacy-core', get_template_directory_uri() . '/css/style.css', ['bootstrap'], '1.0.0' );
    wp_enqueue_style( 'premium-system', get_template_directory_uri() . '/css/premium-system.css', ['bootstrap', 'legacy-core'], '1.2.0' );
    
    // 2. JS (Deferred for performance)
    wp_enqueue_script( 'jquery' ); // WP Core jQuery
    wp_enqueue_script( 'bootstrap', get_template_directory_uri() . '/js/bootstrap.min.js', ['jquery'], '3.3.7', true );
    wp_enqueue_script( 'owl-carousel', get_template_directory_uri() . '/js/owl.carousel.min.js', ['jquery'], '2.3.4', true );
    wp_enqueue_script( 'wow', get_template_directory_uri() . '/js/wow.min.js', [], '1.1.2', true );
    wp_enqueue_script( 'lightslider', 'https://cdnjs.cloudflare.com/ajax/libs/lightslider/1.1.6/js/lightslider.min.js', ['jquery'], '1.1.6', true );
    wp_enqueue_script( 'premium-system', get_template_directory_uri() . '/js/premium-system.js', ['jquery', 'owl-carousel', 'wow'], '1.2.0', true );
} );
...
/**
 * Force WhatsApp chat popup to start CLOSED on page load.
 * Only the small floating icon is visible; user must click it to open.
 * This prevents the popup from blocking hero images and product photos.
 */
add_action( 'wp_head', function () {
    echo '<style id="ea-wa-tame-v2">
/* ── CRITICAL: Force WhatsApp Closed ── */
div[class*="h3-click-to-chat-whatsapp-icon-chat-box"],
div[class*="h3-ctt-whatsapp-chat-body"],
.h3-click-to-chat-whatsapp-icon-chat-box,
.h3-ctt-whatsapp-chat-body,
#h3-ctt-whatsapp-chat-box,
[id*="h3-ctt"] .chat-box,
.h3_anim_btn_hide_chat { display: none !important; opacity: 0 !important; pointer-events: none !important; }

/* Keep icon visible */
.h3-click-to-chat-whatsapp-icon-main { display: flex !important; pointer-events: auto !important; }

/* Show on click */
.h3-click-to-chat-whatsapp-icon-main.h3-open-chat .h3-click-to-chat-whatsapp-icon-chat-box,
.h3-click-to-chat-whatsapp-icon-main.h3-open .h3-click-to-chat-whatsapp-icon-chat-box { 
    display: block !important; 
    opacity: 1 !important; 
    pointer-events: auto !important; 
}

/* ── CRITICAL: Force Off-Canvas / Categories Closed ── */
.navbar-collapse.in, .navbar-collapse.show { display: none !important; }
@media (min-width: 768px) {
    .navbar-collapse { display: block !important; }
}
#myNavbar.collapse:not(.in) { display: none !important; }
</style>' . "\n";
} );
...
/**
 * OPTIMIZATION: Core Updates properly managed.
 * We allow minor security updates (default behavior) but can filter specifically if needed.
 * Disabling updates entirely was flagged as a Phase 1/2 security risk.
 */
...
function orderbypost($query) {
  ...
  if ( !is_admin() && $query->is_main_query() ) {
    ...
    if (is_post_type_archive( array( 'product', 'slider' ) )) {
...
function ea_send_otp_handler() {
    ...
    try {
        $otp = strval( random_int( 100000, 999999 ) ); // Cryptographically secure 2026 standard
    } catch (Exception $e) {
        $otp = strval( mt_rand( 100000, 999999 ) ); // Fallback
    }
