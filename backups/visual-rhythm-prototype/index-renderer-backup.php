...
if ( is_dir( $logo_dir ) ) {
  $logo_files = glob( $logo_dir . '*.{webp,png,jpg,jpeg,svg}', GLOB_BRACE );
  if ( $logo_files ) {
...
body.home #ea-hp-hero, body.page-template-index #ea-hp-hero {
  ...
  padding-top: 0 !important; /* Offset is handled by global JS updateHeader() */
}
...
<section id="ea-hp-hero">
  <div class="ea-hp-hero-inner">
    <div class="ea-hp-hero-content">
      <span class="ea-hp-hero-label"><?php echo $first_slide ? esc_html( $first_slide['eyebrow'] ) : 'Exhibition &middot; Signage &middot; Branding &middot; Dubai &amp; UAE'; ?></span>
      <h1><?php echo $first_slide ? esc_html( $first_slide['heading'] ) : 'Premium Exhibition, Signage &amp; Branding Company in Dubai'; ?></h1>
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    {"@type":"Question","name":"<?php echo esc_js( $faq['q'] ); ?>","acceptedAnswer":{"@type":"Answer","text":"<?php echo esc_js( $faq['a'] ); ?>"}}<?php echo $fi < count( $faqs ) - 1 ? ',' : ''; ?>
    <?php endforeach; ?>
  ]
}
</script>
