...
@set_time_limit(1800);
echo "<h1>🛡️ Phase 2: Watermark-Free Sync</h1>";
echo "<pre>";
...
foreach ($products as $p) {
    if (strpos($p->post_title, 'Standard Business Cards') !== false) {
        // Skip some Ali priorities if needed, but let's just focus on replacing MPrint
    }
    ...
    if (preg_match('/_[0-9]+\.webp$/', $current_file) && strpos($current_file, 'mod_v3') === false) {
        // This is an Ali image or similar high-priority from Phase 1
        $skipped++;
        continue;
    }

    // TARGET: Any image that is a placeholder OR an old MPrint watermark image
    // Old mprint images don't have 'mod_v3' in them.
    $is_watermarked = (strpos($current_file, 'mod_v3') === false && 
    ...
    if ($is_placeholder || $is_watermarked) {
        // Find match in the refined_image_pool (already v3)
        ...
        if ($found) {
            $res = wp_insert_attachment_from_path_v3($found['path'], $p->ID, $p->post_title);
            if ($res) {
                set_post_thumbnail($p->ID, $res);
                $updated++;
                echo "🚀 [Updated] {$p->post_title} -> " . $found['filename'] . "\n";
            }
        }
    }
}

function wp_insert_attachment_from_path_v3($path, $parent_id, $title) {
    if (!file_exists($path)) return false;
    $wp_upload_dir = wp_upload_dir();
    $name = basename($path);
    
    // Check if already in usage
    $existing = get_posts(['post_type'=>'attachment', 'meta_query'=>[['key'=>'_v3_pool_path','value'=>$path]], 'posts_per_page'=>1]);
    ...
    if (copy($path, $new_path)) {
        ...
        update_post_meta($attach_id, '_v3_pool_path', $path);
...
echo "<h2>✅ Watermark-Free Upgrade Complete!</h2>";
echo "Products cleaned/updated: $updated\n";
