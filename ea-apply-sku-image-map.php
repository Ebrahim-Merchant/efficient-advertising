...
$mapCsv = 'C:/Users/merch/New Efficient Advertising Website Refinement v1.0 10042026/public/sku-image-map.csv';
$logJson = 'C:/Users/merch/New Efficient Advertising Website Refinement v1.0 10042026/public/sku-image-assign-report.json';
...
function ea_find_attachment_by_relative_path(string $relativePath): int
{
    $q = new WP_Query([
...
function ea_ensure_attachment(string $sourcePath, string $destPath, string $title, string $alt): int
{
    ...
    $existing = ea_find_attachment_by_relative_path($relative);
    if ($existing > 0) {
        if ($alt !== '') {
    ...
    $meta = wp_generate_attachment_metadata($attachmentId, $destPath);
    if (!is_wp_error($meta)) {
...
while (($row = fgetcsv($in, 0, ',', '"', '\\')) !== false) {
    ...
    $attachmentId = ea_ensure_attachment($imagePath, $destPath, $name !== '' ? $name : $sku, $name !== '' ? $name : $sku);
    if ($attachmentId <= 0) {
